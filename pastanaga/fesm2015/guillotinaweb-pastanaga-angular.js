...
const COLORS$1 = ['primary', 'secondary', 'destructive', 'contrast'];
    ...
    set color(value) {
        if (!!value) {
            ...
            COLORS$1.forEach(color => {
                const colorClass = this.getClassFromInput('color', color, COLORS$1);
...
const getFixedRootParent = (element) => {
...
const getPositionnedParent = (element) => {
    if (element.tagName === 'BODY') {
        return element;
    }
    const style = getComputedStyle(element);
...
const getVirtualScrollParentPosition = (element) => {
    let tmp = element;
...
const markForCheck = (cdr) => {
    if (!cdr.destroyed) {
        cdr.markForCheck();
    }
};
const detectChanges = (cdr) => {
    if (!cdr.destroyed) {
        cdr.detectChanges();
    }
};
...
const getInitials = (username) => {
    let initials = '—';
    if (!!username.trim()) {
        const usernameWords = username.trim().split(' ').map(word => getValidLanguageText(word));
...
const getValidLanguageText = (text) => {
...
const sortCheckboxes = (checkboxes) => {
...
let PastanagaProgressCircleComponent = class PastanagaProgressCircleComponent {
    constructor() {
        this.isLarge = false;
        this.color = 'primary';
        this.percentValue = 0;
...
let DropdownComponent = class DropdownComponent extends PopupComponent {
    constructor(popupService, renderer, element, cdr) {
        super(popupService, renderer, element, cdr);
        this.popupService = popupService;
        this.renderer = renderer;
...
let DropdownSeparatorComponent = class DropdownSeparatorComponent {
};
